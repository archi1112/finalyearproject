...
class FaceRecognition:
    ...
    def faceDetect(self, emp_id):
        """
        Detect faces in the webcam and save them to the dataset folder.
        ...
        while True:
            ...
            #  transforms the image into a format that the neural network can process.
            blob = cv2.dnn.blobFromImage(frame, 1.0, (300, 300), [
                                         104, 117, 123], False, False)

            # Pass the blob through the network and get the detections
            net.setInput(blob)
            # detection is 4d array containing class,confidence scores
    ...
    def trainFace(self):
        ...
        face_recognizer.train(faces, cv2.ml.ROW_SAMPLE, np.array(ids))
            ...
            # Save the model
        face_recognizer.save(BASE_DIR+'/Employee_attendance/trainer/trainer.yml')
        print("model saved")

            # Print the number of faces trained
        print("\n {0} faces trained. Exicv2.FONT_HERSHEY_SIMPLEXting Program".format(len(np.unique(ids))))
